...
std::string CrackCipherSystem::crack(const std::filesystem::path &path_to_file) {
...
void CrackCipherSystem::AlterDecryptText(std::unordered_map<char, char> &changed_chars) {
...
bool caesar_cipher::CrackCipherSystem::TryToChange(std::unordered_map<char, char> &changed_chars) {
  ...
  AlterDecryptText(changed_chars);
  if (GetPercRecognizedWords() < perc) {
